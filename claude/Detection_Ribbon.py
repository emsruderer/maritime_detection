...
def main():
    ...
    with VDevice() as device:
        ...
        if show_window:
            try:
                cv2.namedWindow("Hailo-8 YOLOv8n - Pi5", cv2.WINDOW_NORMAL)
            except cv2.error:
                show_window = False
        if not show_window:
            print("No GUI display detected. Running headless; press Ctrl+C to stop.")
            ...
            with ng.activate(ng_params):
                ...
                while True:
                    if show_window:
                        ...
                        arrays, _ = cam.capture_arrays(["main", "lores"])
                        display_frame = arrays[0]
                        lores_frame = arrays[1]
                    else:
                        # In headless mode, camera is configured as lores-only main stream.
                        lores_frame = cam.capture_array("main")
                        display_frame = None
                    ...
                    if show_window and display_frame is not None:
                        draw(display_frame, boxes, confs, cls_ids)
                    ...
                    fps_t = now
                    if show_window and display_frame is not None:
                        cv2.putText(display_frame, f"FPS {fps:.1f}", (10, 32),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0), 2)

                    if frame_idx % LOG_EVERY_N == 0:
                        print(f"Frame {frame_idx}: detections={len(boxes)} fps={fps:.1f}")

                    if show_window:
                        cv2.imshow("Hailo-8 YOLOv8n - Pi5", display_frame)
